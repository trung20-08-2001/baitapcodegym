...
DELIMITER //
CREATE TRIGGER TINHHOADON AFTER INSERT ON CTHD 
FOR EACH ROW
BEGIN 
UPDATE HOADON
SET TRIGIA =(
    SELECT GIATRI FROM (
        SELECT CTHD.SOHD,SUM(SANPHAM.GIA * CTHD.SL) AS GIATRI
        FROM CTHD
        INNER JOIN SANPHAM ON CTHD.MASP = SANPHAM.MASP
		GROUP BY CTHD.SOHD)AS TABLE_TEMP 
	WHERE TABLE_TEMP.SOHD=NEW.SOHD)
WHERE HOADON.SOHD = NEW.SOHD;
END //
DELIMITER ;

DELIMITER //
CREATE TRIGGER TINHDOANHSOKH AFTER UPDATE ON HOADON
FOR EACH ROW
BEGIN
UPDATE KHACHHANG SET DOANHSO =(SELECT SUM(TRIGIA)
							  FROM HOADON
                              WHERE HOADON.MAKH=NEW.MAKH)
...
-- 17. Tính doanh thu bán hàng của từng tháng trong năm 2023

SELECT MONTH(NGAY) AS `THÁNG`,SUM(TRIGIA) AS `DOANH SÔ`
FROM HOADON
WHERE YEAR(NGAY)=2023
GROUP BY MONTH(NGAY);
...
-- 18. Tìm hóa đơn có mua ít nhất 4 sản phẩm khác nhau.
  ...
  SELECT SOHD 
  FROM (
	    SELECT SOHD,COUNT(MASP) AS COUNTSP
        FROM CTHD GROUP BY SOHD)
  TABLETEMP WHERE TABLETEMP.COUNTSP>=4;
...
SELECT KHACHHANG.MAKH, KHACHHANG.HOTEN, COUNT(*) AS SOLANMUAHANG
FROM KHACHHANG
...
-- 21. Tháng mấy trong năm 2023, doanh số bán hàng cao nhất ?

SELECT MONTH(NGAY) `THÁNG`,SUM(TRIGIA) AS DOANHSOTHANG
FROM HOADON
WHERE YEAR(NGAY)=2023
GROUP BY MONTH(NGAY)
ORDER BY DOANHSOTHANG DESC
LIMIT 1;

-- 22. Tìm sản phẩm (MASP, TENSP) có tổng số lượng bán ra thấp nhất trong năm 2023.
...
INNER JOIN (
    ...
    LIMIT 10
) AS TOP10 ON KHACHHANG.MAKH = TOP10.MAKH
GROUP BY KHACHHANG.MAKH
ORDER BY SOLANMUAHANG DESC
LIMIT 1;
